...
| `verify-job-picker.mjs` | 2026-09-22 | The saved-job picker it covered was removed entirely, along with `src/jobs/` and the `jobs`/`my_jobs` tables. This one had been a **core** check, and the day before it was retired its Settings section was trimmed for the same reason — a good illustration of the rule above: the first trim was right because only part of its subject had gone, and retirement was right once all of it had. Typing a job number by hand is still covered, because that path was never part of this script's subject — it is an ordinary field on the Job Info step. |
